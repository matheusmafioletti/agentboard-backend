...
import com.agentboard.board.security.JwtAuthFilter;
...
class JwtAuthFilterTest {
  ...
  void shouldNotSetSecurityContext_whenInvalidTokenProvided() throws Exception {
    when(jwtValidator.validate("bad-token")).thenThrow(new InvalidTokenException("bad token"));

    MockHttpServletRequest request = new MockHttpServletRequest();
    request.addHeader("Authorization", "Bearer bad-token");
    MockHttpServletResponse response = new MockHttpServletResponse();
    AtomicReference<Object> capturedAuth = new AtomicReference<>();
    MockFilterChain chain = new MockFilterChain() {
      @Override
      public void doFilter(jakarta.servlet.ServletRequest req, jakarta.servlet.ServletResponse res)
          throws java.io.IOException, jakarta.servlet.ServletException {
